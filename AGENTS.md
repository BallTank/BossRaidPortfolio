...
- 파일 링크는 `[파일명:라인](/d:/절대/경로/파일.md#L라인)` 형식의 `markdown link + 절대 로컬 경로`를 기본값으로 사용한다.
- 파일만 가리킬 때는 `[파일명](/d:/절대/경로/파일.md)` 형식을 사용한다.
- 상대 경로(`Assets/...:라인`) 또는 plain text 표기는 위 로컬 링크 형식이 동작하지 않는 경우에만 fallback으로 사용한다.
