...
## 📅 날짜별 로그
- [2026-04-10.md](./2026-04-10.md) - basic/lunge player-hit 원인 분리를 위한 trace instrumentation 추가 후 롤백, same-day follow-up으로 `S05~S12` no-toggle `HitTrace` 로그 + `HitTraceLogger`(콘솔 Warning + 파일 sink + runtime BOOT) 보강, warning close-cause reason trace(`Hide*`, `ForceEnd/FinishPlayback`) 추가, pre-active hide defer + lunge close one-shot guard 적용, basic timing follow-up으로 `readyNormalizedWindow.y` crossing 즉시 active phase open 연결, `BossRaidPortfolio.sln` 빌드 검증, 문서 동기화
