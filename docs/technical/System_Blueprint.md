...
| Component | Note |
| --- | --- |
| **Game Loop** | `TitleSceneController` -> `SceneLoader` -> `LoadingSceneController` -> `GameManager` 흐름으로 title, loading, gameplay, result를 분리한다. solo `GamePlay` target은 explicit full path(`Assets/Scenes/mutiplayer/GamePlayScene.unity`)를 사용한다. multiplayer build flow는 verify/gameplay를 분리해 유지하며, `Fast Multiplayer Verify Build`는 Beautify runtime resources를 제외한 빠른 smoke build, `Fast Multiplayer Gameplay Build`는 full gameplay scene + Beautify 유지 기준 build로 나눈다. |
