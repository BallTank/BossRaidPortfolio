...
| **Audio Runtime** | `SoundController`, `SoundLibrary`, `TitleSceneController`, `GameManager`가 current lightweight audio runtime baseline을 구성한다. title 진입 시 `TitleSceneController`는 필요하면 `GlobalSoundController`를 생성하고 `Resources/SoundLibrary.asset`을 읽어 lobby BGM과 UI press SFX를 준비한다. gameplay/result에서는 `GameManager`와 각 combat/presentation path가 battle/result BGM과 player/boss SFX를 호출한다. 2026-05-14 follow-up 기준 player voice는 `PlayerController.PlayerSoundProfile`로 Player1/Player2 sound id를 분리하고, local-presenting avatar만 voice/dash/hit/katana hit SFX를 재생한다. multiplayer boss display replay도 `MultiplayerBossAuthorityBridge`가 AoE/fireball/fly/scream/death/basic attack sound 계열 SFX를 remote client 쪽에서 같이 재생한다. |
...
| **Multiplayer Runtime Bridge** | `MultiplayerPlayerAvatar`, `MultiplayerBossAuthorityBridge`, `PlayerLocomotionCore`, `MultiplayerPlayerPresentationDriver`가 각각 player authority glue, boss authority bridge, shared locomotion, local presentation을 분담한다. 2026-04-07 result flow follow-up 기준 avatar는 replicated HP/dead뿐 아니라 retry-ready bit와 active avatar registry도 같이 들고, `GameManager`는 이 shared multiplayer bridge surface를 result/retry count source로 재사용한다. same-day cleanup 이후 temporary action trace, disconnect/session continuity debug, boss lunge debug spam은 제거하고 current verify에는 warnings/errors 위주로 남긴다. 2026-04-20 Resources cleanup 이후 runtime player prefab reference는 `Assets/Resources/MultiplayerRuntimeConfig.asset`을 canonical config로 사용한다. 2026-04-29 follow-up 기준 NGO runtime registration은 one shared network player prefab + stable custom prefab hash를 사용하고, Host/Client visual difference는 `MultiplayerPlayerAvatar`가 owner client id를 기준으로 presentation root와 `NetworkAnimator` target을 다시 바인딩하는 방식으로 분리한다. 2026-04-30 multiplayer combat follow-up에서는 `PlayerController`가 same visual refresh path에서 active `DamageCaster`도 다시 찾아 bind해, shared prefab의 serialized `_damageCaster = null` 상태와 owner-based visual swap이 겹쳐도 active katana hitbox를 runtime에 복구한다. 2026-05-01 host look fix에서는 `MultiplayerRuntimeConfig.asset`이 explicit `HostVisualTemplate` source를 들고, `MultiplayerPlayerAvatar`가 이 template를 runtime `HostVisual` child로 복제해 host presentation source를 shared prefab default look과 분리한다. same-day solo diagnosis follow-up에서는 shared gameplay scene의 solo bootstrap 부재 여부를 읽기 위해 `GameManager`, `LocalInputProvider`, `PlayerController`가 `[SoloDebug]` 로그로 scene avatar active state, input enable, runtime simulation/authority state를 surface한다. same-day avatar visibility follow-up에서는 solo no-active-session path만 `GameManager`가 `SoloPlayerAvatar`를 켜고 `MultiPlayerAvatar`를 끄며, active multiplayer session에서는 `MultiplayerGameplaySceneCoordinator`와 `MultiplayerPlayerAvatar` scene-object path가 `SoloPlayerAvatar`/`MultiPlayerAvatar` root를 marker-only disabled 상태로 고정한다. same-day movement replay follow-up에서는 `MultiplayerPlayerAvatar`가 visual swap 뒤 `NetworkAnimator` internal cache를 current active animator 기준으로 재생성해 client 화면의 host movement animation 누락을 줄인다. 2026-05-14 follow-up에서는 avatar가 owner role 기준 sound profile을 주입하고, dash active edge를 local/server에서 감지해 local dust play와 remote `PlayDashDustClientRpc` replay를 분리한다. gameplay scene spawn pose는 `GamePlayScene_Verify.unity`와 `GamePlayScene.unity` 둘 다 exact scene prefab instance 대신 plain marker(`SoloPlayerAvatar`, `MultiPlayerAvatar`) transform에서 읽는다. |
...
| **Attack Warning Ownership** | Attack1/Attack2는 body/head `DamageCaster` 대신 `AttackWarningController`를 source-of-truth로 사용한다. Attack1은 sector warning + one-shot sector damage, Attack2는 strip warning + fixed-distance strip damage를 사용한다. Attack1 tuned hide는 warning duration이 실제로 끝난 뒤에만 full end를 허용해 pre-active `ForceEnd()` regression을 막는다. pre-active hide 요청은 `DEFER_PREACTIVE`로 처리해 active phase 진입 전 강제 종료를 막고, lunge close는 one-shot guard로 동일 hide 호출 스팸을 차단한다. Basic은 `readyNormalizedWindow.y` crossing frame에서 `TryEnterBasicAttackTelegraphActiveNow(...)`를 직접 호출해 Attack1 damage open을 ready normalized 경계에 맞춘다. 2026-05-14 follow-up에서는 `DragonAttack1` sound도 same ready-end boundary에서 one-shot으로 재생하고 `BossReplicatedEffectKind.BasicAttackSound`로 remote client에 전달한다. latest warning visibility follow-up에서는 `BossController` main `Attack Settings`의 `warningVisualHeightOffset` one master field가 Attack1/2/4 visual child Y lift를 함께 조정하고, damage query center/root position은 그대로 유지한다. particle-only warning prefab child가 auto-bound되면 `AttackWarningController`는 same path에서 runtime fallback mesh를 사용하고, fallback active 동안 source particle/VFX child를 잠시 숨겨 Basic/Lunge visibility를 보장한다. latest keep-state에서는 Basic만 plain runtime `BasicAttackWarning_Runtime` telegraph를 생성해 `AoECircleController` visual reset path를 우회하고, first-use replay warm-up issue는 portfolio scope accepted limitation으로 남긴다. legacy `HeadDamageCaster` / `LungeDamageCaster` reference는 old anchor fallback 용도로만 남긴다. |
...
| **Boss Authority Contract** | current `step 1-2` 기준 `BossController`는 `CaptureAuthoritativeState(...)`로 dedicated boss snapshot을 만들고, `MultiplayerBossAuthorityBridge`가 이 snapshot을 Host runtime-root에서 capture/send 한다. snapshot은 transform / locomotion state / current attack id / current attack visual state / attack start server tick / current attack `normalized time` / current attack playback speed / HP / phase / dead flag를 담는다. `AttackStartServerTick`는 fallback attack clock으로 유지하고, basic ready-slice처럼 host animator speed override가 있는 경우에는 actual animator progress가 추가 source-of-truth가 된다. 2026-05-14 follow-up 기준 attack start tick은 한 번 positive value로 resolve되면 같은 attack 동안 cached value를 재사용해 snapshot replay clock drift를 줄인다. AoE airborne replay follow-up에서는 same snapshot의 `AttackVisualState`가 `TakeOff / FlyForward / FlyIdle / Land` semantic phase를 client에 직접 전달한다. 2026-05-01 boss hit replay follow-up에서는 bridge hit apply가 `BossVisual.TriggerHit()`와 함께 `BossController.PlayDamageBlinkVisual()`을 호출해 client에서도 boss white blink를 같은 진입점으로 재생한다. |
| **Boss Multiplayer Read** | multiplayer client는 boss gameplay truth를 직접 계산하지 않고, disabled local boss object에 `MultiplayerBossAuthorityBridge`가 latest dedicated state의 display-only transform/semantic animator state를 apply하는 구조를 기준으로 한다. current baseline은 direct apply이며, same received boss tick은 한 번만 consume하고, move/search locomotion speed는 weak packet delta에서도 host locomotion speed를 바닥값으로 유지한다. latest Basic replay follow-up에서는 pending replicated effect/warning을 same frame state apply보다 먼저 소비해 red warning start를 앞세우고, warning이 아직 local에서 running이 아닐 때만 `Basic Attack` visual에 `0.08s` small grace를 적용해 `AttackNormalizedTime / AttackPlaybackSpeed` snapshot replay가 warning보다 먼저 달려 보이는 현상을 완충한다. current keep-state에서는 Basic remote warning object를 first use 때 plain runtime telegraph로 한 번 생성하고 이후 cached object를 재사용한다. AoE는 `AttackVisualState` 변화를 기준으로 `TakeOff -> FlyForward -> FlyIdle -> Land` 비행 phase를 순서대로 재생하며, 2026-05-14 follow-up 기준 duplicate/backward visual state는 skip해 client airborne animation을 되감지 않는다. boss HUD도 같은 bridge가 `CurrentHealth/MaxHealth` snapshot을 local `CombatHUDController`에 직접 써서 local disabled boss `Health`와 분리한다. same bridge는 `TryGetLatestBossState(...)` / `IsBossDead` read surface로 later result flow의 boss truth source도 제공한다. client gameplay prediction/extrapolation은 넣지 않는다. |
| **Boss Effect Replay** | attack 1/2 warning과 attack 3/4의 spawned projectile/AoE visual은 `BossReplicatedEffectEvent`로 별도 전송한다. Host는 basic sector / lunge strip warning의 `show/hide`, projectile shot, AoE spawn, Basic attack sound를 same reliable named message batch에 큐잉하고, `MultiplayerBossAuthorityBridge`가 remote client에 보낸다. client는 local `AttackWarningController`, pooled `BossProjectile`, `AoECircleController`, `SoundController`를 display-only로 재생한다. damage truth는 계속 Host-only다. |
...
| **Title Prototype UI** | `TitleSceneController`는 `TitleMainPanel`, multiplayer 선택 패널, join/lobby/wrong-key panel을 title flow 검증용으로 관리한다. source snapshot 보관은 `Assets/Scenes/merged/TitleScene.unity`, runtime target은 `Assets/Scenes/mutiplayer/TitleScene.unity`를 사용한다. runtime 시작은 `Text_PressAnyKey`를 먼저 보여주고 keyboard/mouse 입력 1회 후 panel을 연다. runtime root는 `TitleRuntimeRoot (1)`을 먼저 바인딩하고 없을 때만 `TitleRuntimeRoot`로 fallback한다. selected root는 Play Mode에서 center anchor + local zero 계약으로 정규화되고 root Animator는 비활성화해 quarter/right-half 표시 회귀를 막는다. `TitleMainPanel`은 Main state 진입 시 runtime 강제 재정렬 없이 scene 배치값을 유지한다. same prompt는 scene anchor/position 기준 하단 중앙 배치를 유지하고, recovered animation controller가 anchored position을 덮어쓰지 않도록 Animator를 비활성으로 유지한다. background layer는 `TitleCanvas/Background/Image` 오브젝트를 활성 상태로 유지하고 Animator만 비활성으로 잠가 child decorative mesh가 `Image_title (1)` 위치로 떠오르는 회귀를 막는다. latest multiplayer follow-up에서는 `MultiplayerTitleSceneDriver`를 runtime에서 self-heal attach하고, button bind scope를 preferred runtime root subtree로 제한해 duplicate button name 오바인딩을 막는다. 2026-05-12 audio follow-up에서는 title/multiplayer title button bind path가 `UiPressSoundHook`를 함께 붙여 UI press SFX를 공통 경로로 재생한다. |

### 3.5. Game Logic & Flow
| Component | Note |
| --- | --- |
| **Game Loop** | `TitleSceneController` -> `SceneLoader` -> `LoadingSceneController` -> `GameManager` 흐름으로 title, loading, gameplay, result를 분리한다. solo `GamePlay` target은 explicit full path(`Assets/Scenes/mutiplayer/GamePlayScene.unity`)를 사용한다. multiplayer build flow는 verify/gameplay를 분리해 유지하며, `Fast Multiplayer Verify Build`는 Beautify runtime resources를 제외한 빠른 smoke build, `Fast Multiplayer Gameplay Build`는 full gameplay scene + Beautify 유지 기준 build로 나눈다. 2026-05-01 solo diagnosis follow-up에서는 multiplayer title scene의 solo button이 `MultiplayerTitleSceneDriver` direct load path를 타는지 확인하기 위해 title click에도 `[SoloDebug]` console trace를 남긴다. |
