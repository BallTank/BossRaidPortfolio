...
| **Attack Warning Ownership** | Attack1/Attack2는 body/head `DamageCaster` 대신 `AttackWarningController`를 source-of-truth로 사용한다. Attack1은 sector warning + one-shot sector damage, Attack2는 strip warning + fixed-distance strip damage를 사용한다. Attack1 tuned hide는 current build에서 warning duration이 실제로 끝난 뒤에만 full end를 허용해 pre-active `ForceEnd()` regression을 막는다. 2026-04-10 verify follow-up에서는 원인 분리 목적의 임시 no-toggle `HitTrace`를 `S05~S12` 단계(`ResolveDamageable -> TargetFilter -> ReceiverCall -> PlayerGate -> HealthReject`)로 추가했고, same-day logger 보강으로 콘솔 Warning + `Application.persistentDataPath/HitTrace.log` 파일 sink + runtime BOOT trace를 같이 남긴다. same-day close-cause 보강으로 `HideBasic/LungeAttackTelegraph(reason)`와 `ForceEnd/FinishPlayback(reason)` trace를 추가해 warning이 active phase 전에 닫힐 때 caller reason을 역추적할 수 있다. same-day fix에서는 pre-active hide 요청을 `DEFER_PREACTIVE`로 처리해 active phase 진입 전 강제 종료를 막고, lunge close는 one-shot guard로 동일 hide 호출 스팸을 차단한다. latest follow-up에서는 Basic이 `readyNormalizedWindow.y` crossing frame에서 `TryEnterBasicAttackTelegraphActiveNow(...)`를 직접 호출해 Attack1 damage open을 ready normalized 경계에 맞춘다. legacy `HeadDamageCaster` / `LungeDamageCaster` reference는 old anchor fallback 용도로만 남긴다. |
