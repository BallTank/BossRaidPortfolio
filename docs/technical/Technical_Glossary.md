...
* **Separate Action-Intent Path**: locomotion input packet과 분리된 multiplayer action uplink 경로. initial implementation step에서는 `dash / Attack1` 입력 edge만 별도 packet으로 Host에 보내는 형태로 시작했고, 2026-04-03 same-day combat follow-up에서는 이 same path를 `Attack2 / Attack3` combo continuation press와 attack-cancel dash request까지 넓혔다. movement packet과 action/result packet을 섞지 않는 계약 자체는 그대로 유지한다. `FacingYaw`도 action edge 시점에 함께 실어, Host execute와 owner apply가 같은 facing source를 공유한다.
* **HostPlayerState**: Host가 player 1명에 대해 유지하는 authoritative runtime state struct. `HP`, active action, accepted action sequence/start tick, active/accepted combo step, consumed reaction sequence, hit/stun/death flag를 담고 이후 replication의 source state가 된다. 2026-04-03 `step 9` boundary freeze 기준 current runtime에서는 아직 continuous full-state packet 자체가 아니라, Host-side truth/source struct로 해석한다.
* **HostToClientPlayerReactionSnapshot**: Host가 one-shot reaction result를 전달하기 위해 만드는 snapshot struct. `reaction sequence`, `server tick`, `damage`, `result HP`, `source hit type`, `interrupted action`을 함께 담아 later client dedupe/apply의 기준이 된다. 2026-04-03 `step 8` follow-up 기준 owner client는 이 `reaction sequence`로 duplicate hit/stun/death replay를 막는다.
...
* **Hybrid Send Path**: current `step 9` temporary send layout. `MultiplayerPlayerAvatar`가 owner locomotion RPC, owner approved action start RPC, owner reaction snapshot RPC, HUD `NetworkVariable<int>` pair, local-derived label resolve, remote `NetworkTransform / NetworkAnimator` display sync를 함께 들고 있는 상태다. dedicated `HostToClientPlayerStateReplicator` 이전의 verify answer로 쓴다.
* **Temporary Send Boundary Freeze**: current `step 9` documentation rule. new `Replicator` class를 바로 만들지 않고, 어떤 값이 어떤 temporary path로 가는지 먼저 고정해 later `one packet / one path` cleanup scope를 줄이는 절차다.
* **Mixed Apply Path**: current `step 10` temporary apply layout. dedicated `HostToClientPlayerStateApplier` 대신 `MultiplayerPlayerAvatar`가 owner runtime apply, owner HUD apply, remote display-only rule을 함께 들고 있는 상태다.
* **Owner Runtime Apply**: owner client가 Host result를 실제 player runtime에 반영하는 current path 묶음. `ApplyLocomotionState(...)`, `ApplyAuthoritativeActionStartLocally(...)`, `ApplyAuthoritativeReactionLocally(...)`가 여기에 속한다.
* **Owner HUD Apply**: local-owned avatar가 자기 화면의 `CombatHUDController`에 self/partner HP와 viewer-side label을 다시 쓰는 current viewer-side HUD mapper path. gameplay truth 계산이 아니라 Host 기준값 표시가 목적이다.
* **Temporary Apply Boundary Freeze**: current `step 10` documentation rule. owner runtime apply와 remote display-only path를 먼저 고정해 later `Applier` cleanup에서 gameplay truth와 display-only를 다시 섞지 않게 만드는 절차다.
* **Visual-Only Presentation Freeze**: current `step 11` documentation rule. `MultiplayerPlayerPresentationDriver`를 owner local visual polish helper로만 읽고, gameplay truth / FSM authority / HP-result apply와 분리해 later cleanup에서도 hidden authority path가 되지 않게 만드는 절차다.
...
* **ActionAuthorityMode**: `PlayerController` runtime gate. `SoloLocal / HostAuthoritative / ClientOwnerProxy / RemoteDisplayOnly` 네 역할을 사용해 local input이 attack/dash state를 직접 시작해도 되는지, local boss-hit reaction을 직접 처리해도 되는지, local hitbox를 켜도 되는지를 나눈다. `step 8` 기준 multiplayer attack start는 solo를 제외하고 모두 Host approve path를 먼저 타고, 2026-04-03 same-day combat follow-up에서는 `Attack2 / Attack3` combo continuation과 attack-cancel dash도 같은 원칙으로 확장됐다. same-day facing follow-up 기준 attack start 회전은 stale local frame cache가 아니라 accepted `FacingYaw` pending path를 우선 사용한다.
* **Authoritative Combo Continuation**: owner의 second/third attack press를 local combo reserve로 바로 확정하지 않고, separate action-intent로 Host에 보낸 뒤 Host가 combo window와 next step 존재 여부를 검사해 `Attack2 / Attack3` start를 승인하는 규칙. owner client는 approved action start apply 뒤에만 next combo step을 연다.
* **Authoritative Attack-Cancel Dash**: attack 중 dash press를 local direct cancel로 처리하지 않고, Host가 dash cancel window와 cooldown/state gate를 검사한 뒤 `AttackState -> DashState`를 authoritative하게 시작하는 규칙. owner client는 approved dash start apply 뒤에만 runtime dash로 전환한다.
* **Boss Client Presentation Mirror (제거된 임시 경로)**: 이전 verify unblock path. 과거에는 `MultiplayerPlayerAvatar`가 boss `position / rotation / animator state / normalized time / Speed` snapshot을 owner client로 보내고 local disabled boss object에 apply했다. current `step 2` 이후 이 path는 boss truth writer에서 제거됐고, historical 용어로만 남는다.
* **BossAuthoritativeState**: boss dedicated authority snapshot struct. Host는 `BossController.CaptureAuthoritativeState(...)`를 통해 boss gameplay runtime에서 직접 `position / rotation / locomotion state / current attack id / attack start server tick / HP / phase / dead flag`를 한 DTO로 읽고, `MultiplayerBossAuthorityBridge`가 이 DTO를 client로 보낸다. current scope에서는 projectile/AoE 전용 extra field는 넣지 않는다.
* **BossReplicatedEffectEvent**: attack 3 projectile shot과 attack 4 AoE spawn을 remote client에 display-only로 재생시키기 위한 small effect DTO. `EffectKind / SequenceId / StartPosition / Direction / ImpactPosition / Speed / Lifetime / WarningDuration / ActiveDuration / Radius / TargetNetworkObjectId`를 담고, state snapshot과 분리된 one-shot event로 보낸다.
* **Boss Attack Start Bookkeeping**: `BossAttackState.Enter()/Exit()`와 `BossController.HandleDeath()`에서 active attack id와 attack start time을 기록/정리하는 small runtime record. animator state hash가 아니라 real gameplay attack boundary를 later replicated `AttackStartServerTick`의 source로 쓰기 위한 준비 단계다.
* **MultiplayerBossAuthorityBridge**: `MultiplayerRuntimeRoot`에 붙는 boss dedicated multiplayer bridge. Host는 network tick마다 `BossAuthoritativeState`를 capture해 NGO named message로 remote client에 보내고, client는 local disabled boss object를 찾아 transform과 semantic animator state를 display-only로 apply한다. 2026-04-06 same-day `step 3` smoothing 시도는 client jitter regression 때문에 rollback됐고, current baseline은 direct apply 기준이다. same-day walk follow-up에서는 같은 received boss tick을 한 번만 consume하도록 gate를 두고, move/search locomotion speed는 weak packet delta에서도 host locomotion speed를 바닥값으로 유지하도록 보강했다. same-day projectile/AoE follow-up에서는 second reliable named message로 `BossReplicatedEffectEvent` batch를 보내 attack 3 projectile과 attack 4 red circle/falling fire를 remote client에 display-only replay하게 확장됐다. 2026-04-07 boss HUD follow-up에서는 Host/Client local `CombatHUDController`에 `CurrentHealth/MaxHealth`를 직접 써서 multiplayer boss HP bar가 local disabled boss `Health`를 읽지 않게 정리했다. same-day result follow-up에서는 latest boss snapshot을 local cache로도 들고, `TryGetLatestBossState(...)` / `HasLatestBossState` / `IsBossDead` read surface를 통해 `GameManager`가 multiplayer `Victory` truth를 same authoritative source에서 읽게 했다. separate boss prefab/network actor를 늘리지 않고도 `MultiplayerPlayerAvatar`에서 boss send/apply responsibility를 분리하기 위한 current answer다.
* **Boss Move Interpolation Buffer (historical)**: `MultiplayerBossAuthorityBridge` client path에 previous/current transform target과 interpolation window를 두어 visible transform을 부드럽게 재생하려던 same-day `step 3` 시도. current verify에서는 client boss jitter regression이 확인되어 rollback됐고, historical term으로만 남는다.
...
* **PredictionReconciliation**: current multiplayer movement runtime path의 큰 이름. owner client는 locomotion input을 즉시 predict하고, Host authority replica는 shared `PlayerLocomotionCore`로 같은 입력을 authoritative하게 sim한 뒤 snapshot을 내려준다. 2026-04-01 follow-up 기준 dash도 이 shared locomotion sim 안에서 계산하며, owner는 `walk + dash` 동안 prediction을 유지한다. `Attack / Jump` 같은 non-locomotion input은 fallback/boundary 쪽 authoritative path로 내린다. 2026-04-03 `step 8` 기준 attack start(`Attack1` 시작으로 도입) 자체는 local direct FSM transition이 아니라 Host approve 후 owner-targeted apply path로 시작하고, owner는 accepted start tick 차이만큼 local attack timer를 조금 앞당겨 late exit를 줄인다. same-day combat follow-up에서는 이 approved action-start contract가 `Attack2 / Attack3` combo continuation과 attack-cancel dash까지 넓어졌다. same-day facing follow-up에서는 `ClientToHostPlayerActionIntent.FacingYaw`를 Host execute와 owner apply가 함께 사용해 first attack yaw mismatch를 줄였다. same-day dash visual follow-up에서는 owner render proxy가 whole dash 동안 snap하지 않고, dash start one-shot snap 뒤에는 interpolation으로 이어지게 조정했다. same-day dash-exit follow-up에서는 shared locomotion state가 no-input 마지막 dash tick의 residual `PlanarVelocity`를 0으로 저장해, owner apply/replay가 phantom walk를 다시 재생하지 않도록 맞췄다. 2026-03-30 lazy correction 1차 이후 owner는 free move snapshot을 우선 `shadow authoritative state` 로 저장하고, baseline/fallback/boundary/hard-fail/stale 시점에서만 immediate sync + replay를 수행한다.
* **Runtime Role Configuration Log (historical)**: 과거 `MultiplayerPlayerAvatar` spawn 시 남기던 one-line diagnostic log. `predictionPath=PredictionReconciliation / server / owner / mode`를 출력했지만, 2026-04-06 same-day cleanup에서 제거됐다. old console log를 읽을 때만 historical term으로 참고한다.
* **MultiplayerActionIntentTrace (historical)**: 과거 `MultiplayerPlayerAvatar`가 dash/attack action edge, Host validator/state, reaction snapshot, raw hit log 결과를 남길 때 쓰던 diagnostic log. `observe`, `validate`, `host-state`, `reaction-snapshot`, `raw-hit-log`, `buffer-observe`, `rpc-received` phase를 사용했지만, 2026-04-06 same-day cleanup에서 제거됐다. current verify console은 이 high-volume trace 대신 warnings/errors 위주로 읽는다.
* **MultiplayerConnectionDebug (historical)**: 과거 `MultiplayerSessionService`가 남기던 session continuity structured logger. `peer-connect`, `peer-disconnect`, `state-change`, `gameplay-sync-complete`, `pulse`, avatar profile dump, `noActionObservedAffected` summary를 포함했지만, 2026-04-06 same-day cleanup에서 제거됐다. old disconnect investigation이나 2026-04-02 기록을 읽을 때만 historical term으로 참고한다.
...
* **Gameplay Restart Reload**: defeat retry 합의 뒤 Host가 `MultiplayerSessionService.RestartGameplayAsync()`로 same gameplay scene을 NGO `LoadScene(..., LoadSceneMode.Single)`로 다시 여는 재시작 경로. reload 전 `MultiplayerGameplaySceneCoordinator.DespawnAuthoritativePlayers()`가 old player object를 정리하고, load 완료 뒤 authoritative player spawn을 다시 시작한다.
* **Result Panel Image Contract**: `GameManager`가 multiplayer result를 표시할 때 `GameOver_Panel` root만 켜는 것이 아니라, child `Image_Win` / `Image_Lose` art와 `Text_GameResult` root/TMP labels를 함께 토글하는 UI 계약. verify scene는 `Victory` title을 `Image_Win`, `Defeated` title을 `Image_Lose` art로 기대하고, defeat prompt text만 별도 TMP로 내려간다.
* **Result Death Read Surface**: `MultiplayerPlayerAvatar.TryGetResultDeathState(...)`가 제공하는 result 전용 dead polling helper. replicated HUD HP pair를 먼저 읽고, 필요하면 local dead state나 Host local `Health`를 fallback으로 써서 `GameManager` both-dead 판정 source를 more stable하게 만든다.
* **Dead-Player Spectator Follow**: `ThirdPersonCameraController` multiplayer follow-up rule. local avatar가 dead이고 partner가 alive면, dead player는 local death 뒤 약 `2.5초`를 기다린 다음 자신의 local orbit/look 입력은 유지한 채 alive partner의 `GetPreferredCameraFollowPosition()`을 따라간다. alive player의 camera/input ownership은 변경하지 않는다.
* **Latched Multiplayer Retry Count**: defeat panel retry prompt가 live avatar recount transient state를 그대로 보지 않도록, `GameManager`가 same defeat round 동안 highest valid ready count를 붙잡아 두는 UI stabilization rule. current follow-up에서는 `0/2 -> 1/2 -> 2/2` progression은 허용하지만, once shown count는 same round에서 `1/2 -> 0/2`처럼 뒤로 내려가지 않는다.
* **Disconnect Input Profile (historical)**: 과거 `MultiplayerPlayerAvatar`가 owner input / Host buffered input을 따라가며 누적하던 disconnect 전용 입력 요약. `idle-only`, `idle-walk-only`, `action-observed` label과 `avatar-profile-baseline` / `avatar-profile-transition` event를 사용했지만, 2026-04-06 same-day cleanup에서 제거됐다.
...
* **HUD 부트스트랩 바인딩**: `PlayerController.InitializeCombatHUD()`에서 HUD 참조/보스 `Health`를 초기 탐색한 뒤 `Initialize`, 이름 라벨 세팅, partner HUD visibility gate 적용, combo default hide까지 한 번에 수행하는 시작 절차. solo에서는 local player/boss `Health`를 HUD source로 넘기고, multiplayer에서는 player/boss source를 모두 `null`로 둔 manual mode로 전환한다.
...
* **Boss HUD Snapshot Apply**: multiplayer boss HP bar가 local boss `Health` polling/event 경로를 읽지 않고, `MultiplayerBossAuthorityBridge`가 `BossAuthoritativeState.CurrentHealth/MaxHealth`를 `CombatHUDController.SetBossHpNormalized(...)`에 직접 쓰는 current HUD sync path. Host와 Client가 같은 boss HP source를 보게 만드는 목적이다.
...
* **Multiplayer HUD Health Replica (멀티플레이 HUD 체력 복제본)**: `MultiplayerPlayerAvatar`가 server-authoritative player `current/max HP`를 HUD 전용 `NetworkVariable<int>` pair로 유지하는 경량 스냅샷. gameplay truth를 다시 계산하는 용도가 아니라, Host/Client 양쪽 화면의 player HUD가 같은 HP source를 읽게 만드는 display sync 경로다. 2026-04-03 `step 9` boundary freeze 기준 current separate temporary path로 유지하고, `step 10` apply freeze 기준 actual HUD write는 local-owned avatar의 viewer-side mapper가 맡는다. 2026-04-07 result Step 1 follow-up에서는 same replica를 `TryGetReplicatedHealth(...)` / `IsReplicatedDead` read surface로도 노출해 later result flow가 별도 player HP sync path를 만들지 않고 same authoritative HUD source를 재사용하게 했다.
...
* **Viewer-Side HUD Portrait Layout (화면 기준 HUD portrait 배치 규칙)**: multiplayer HUD portrait도 label과 같은 viewer-relative rule을 따르는 규칙. `CombatHUDController`는 prefab 기본 portrait pair를 Host/Client 기준 sprite source로 캐시하고, local-owned `MultiplayerPlayerAvatar`가 local viewer가 Host인지 Client인지에 따라 좌측 main portrait와 partner portrait를 swap한다. Host 화면은 Host portrait가 좌측, Client 화면은 Client portrait가 좌측에 유지되며, unused legacy partner portrait object는 hidden 상태로 둔다.
...
* **MultiplayerPlayerAvatar**: gameplay scene에서 각 network player object의 runtime role을 정하는 `NetworkBehaviour`. Host-owned authority replica, client-owned local presentation, remote proxy를 role별로 설정한다. runtime hierarchy에서는 `hostPlayer` / `clientPlayer` name으로 role을 구분한다. 2026-04-03 `step 8` follow-up 기준 이 클래스는 role configure 시 `PlayerController.ActionAuthorityMode`도 함께 세팅하고, Host-approved action start / reaction snapshot을 owner-targeted `ClientRpc`로 apply하는 temporary execute path를 가진다. same-day combat follow-up에서는 이 approved action start 범위가 `Attack2 / Attack3` combo continuation과 attack-cancel dash까지 넓어졌다. 2026-04-06 boss authority `step 2` 이후 boss send/apply responsibility는 이 클래스에서 제거됐고, boss path는 `MultiplayerBossAuthorityBridge`가 담당한다. 2026-04-07 result flow follow-up에서는 replicated HUD HP pair를 `TryGetReplicatedHealth(...)`, `HasReplicatedHealthState`, `IsReplicatedDead` read surface로 열고, same avatar object에 retry-ready bit(`IsRetryReady`, `SubmitRetryReadyIfOwner()`)와 active avatar registry도 추가해 `GameManager`가 both-dead defeat와 `(x/2)` retry count를 same spawned player set에서 계산하게 했다.
...
* **Remote Display-Only Apply**: remote client가 Host가 복제한 player state/result를 display/HUD 용도로만 반영하고, runtime gameplay FSM apply는 하지 않는 계약. current multiplayer player action authority 문서에서 first implementation step 기본값으로 채택했다. 2026-04-03 `step 8` 기준 temporary runtime apply는 owner client까지만 열고, remote partner display cleanup은 later `#9-10` 단계로 남겨 둔다. same-day `step 9-10` boundary freeze 기준 current remote partner `dash / Attack1 / Attack2 / Attack3 / hit / stun / death` 표현은 dedicated player-state packet보다 `NetworkTransform / NetworkAnimator` display sync path 해석이 더 맞고, remote replica는 `SimulationMode.Disabled + RemoteDisplayOnly` 상태를 유지한다.
...
* **Aggro Priority Retarget (어그로 우선 재타겟)**: current boss target rule. `BossController.RefreshClosestLiveTarget(...)`는 first acquire fallback으로 현재 씬의 살아있는 `PlayerController` 중 closest live player를 고른다. 하지만 current target이 `AggroPriorityRange` 안에 있는 동안에는 attack 전/후에도 그 target을 계속 유지한다. boss가 어느 플레이어에게서든 첫 피격을 받으면 `AggroTime`이 시작되고, 타이머 동안 current target을 유지한 채 cycle damage를 누적한다. 타이머 종료 뒤 두 플레이어가 `AggroPriorityRange` 안에 함께 있으면 current cycle에서 더 많은 damage를 넣은 player를 damage-lock target으로 고르고 즉시 넘긴다. 현재 target이 우선 원 밖으로 나가거나 invalid/dead가 되면 closest fallback으로 다시 target을 잡고, 다음 피격이 들어오면 새 cycle을 다시 시작한다.
* **Aggro Refresh Split (어그로 갱신 분리)**: `BossController.RefreshClosestLiveTarget(...)`의 internal refactor shape. current implementation은 `ShouldRefreshClosestLiveTarget(...) -> ScanLiveTargets(...) -> ResolveTargetByAggro(...) -> ApplyTargetIfChanged(...)` 순서로 나뉘어, 갱신 간격 제한 / 플레이어 스캔 / 우선순위 판정 / 실제 타겟 적용 책임을 분리한다. gameplay rule을 바꾸는 기능이 아니라, same aggro rule을 더 읽기 쉽게 유지하기 위한 구조 정리다.
* **Client-Side Boss Authority Disable**: multiplayer client가 local `BossController`와 local `CharacterController`를 함께 끄고, boss gameplay truth를 더 이상 local scene object가 쓰지 않게 만드는 verify safety rule. 2026-04-06 boss authority `step 2` 기준 current display restore는 `MultiplayerBossAuthorityBridge`가 담당하고, local boss logic / physics는 계속 disabled로 유지한다.
...
* **동시 사망 판정 우선순위 (GameManager)**: solo에서는 `GameManager.LateUpdate()`가 `_bossDead`를 먼저 검사해 결과를 확정하므로, 플레이어와 보스가 동시에 사망하면 `Victory`를 반환한다. multiplayer에서는 local `_playerDead` 하나를 바로 쓰지 않고, `MultiplayerBossAuthorityBridge.IsBossDead`를 먼저 본 뒤 active `MultiplayerPlayerAvatar` 둘 다 dead일 때만 `Defeated`를 확정한다.
* **Multiplayer Retry Ready Count**: multiplayer defeat panel 아래에 붙는 `Press Enter to Play (x/2)` 합의 카운터. 각 local player는 own avatar에 retry-ready bit를 올리고, Host/Client 양쪽 `GameManager`는 active avatar set을 읽어 same ready count text를 표시한다. `2/2`가 되면 Host가 gameplay restart reload를 시작한다.
...
* **Pattern Attack Range (패턴별 공격 사거리)**: `BossController`가 공격 패턴별 거리 source를 가지는 규칙. current boss는 `Basic`, `Lunge`, `SharedRanged`를 사용하고, Phase2의 `Projectile`/`AoE`는 같은 `SharedRangedAttackRange`를 공용 source로 읽는다.
* **Aggro Priority Circle (어그로 우선 원)**: `BossController.AggroPriorityRange`가 그리는 new detection circle. 두 플레이어가 모두 이 원 안에 있을 때만 `AggroTime` 종료 뒤 damage-lock compare가 열린다. inspector에서는 detection-related tuning이므로 `Detection Settings` 그룹에 둔다.
* **Aggro Time (어그로 시간)**: `BossController.AggroTime` 값. boss가 어느 플레이어에게서든 첫 피격을 받은 뒤 damage contribution을 몇 초 동안 current cycle에 누적할지 정하는 timer다. 이 timer가 도는 동안 boss는 closest rule로 흔들리지 않고 current target을 유지하며, timer가 끝나면 clear damage winner가 있을 때만 target을 교체한다. 다음 cycle은 다음 피격에서 다시 시작한다. inspector에서는 standalone timing control이므로 `Aggro Settings` 그룹에 단독으로 둔다.
* **Shared Ranged Attack Range (공용 원거리 사거리)**: Phase2 projectile/AoE entry gate가 함께 쓰는 단일 사거리 source. 기존 `ProjectileAttackRange`, `AoEAttackRange` 두 원 대신 하나의 ranged circle만 유지해 gizmo/readability를 단순화한다.
* **Boss Damage Contribution Feed (보스 피해 기여 피드)**: `DamageCaster -> BossHitBox -> BossController.RegisterAggroContribution(...)`로 이어지는 small boss-only source reporting path. player가 boss hurtbox에 valid damage를 넣었을 때 dealer identity와 damage amount를 aggro buffer에도 같이 적재한다.
* **Aggro Timer Pause Rule (어그로 타이머 정지 규칙)**: boss가 `AttackState` 또는 phase intro animation 중일 때 `AggroTime` countdown을 멈추는 보호 규칙. timer expiry가 attack start / scream transition을 같은 frame에 끊어 target을 바꾸지 않게 한다.
...
* **Basic Range Single Source (기본 사거리 단일 source)**: Attack1의 조정 가능한 사거리 값은 `BossController.basicAttackRange`를 source-of-truth로 사용한다. `BossController.BasicAttackRange`, Basic gizmo, Phase1 Basic entry gate가 이 값을 읽고, `BossController`는 `OnValidate`/`Start`에서 같은 값을 `HeadDamageCaster.radius`에 동기화해 실제 타격 판정 반경을 맞춘다. inspector에서는 `Detection Settings` 안에서 `DetectionRange`와 `LungeAttackRange` 사이에 둔다.
...
* **Lunge Root Motion Scale Probe (historical)**: 과거 Lunge 튜닝 단계에서 실제 프레임 이동량 배수를 로그로 읽기 위해 쓰던 diagnostic hook. `LungeDebug` 계열 로그와 함께 2026-04-06 same-day cleanup에서 제거됐고, old investigation 문서를 읽을 때만 참고한다.
...
* **Range-Only Detection Trigger (거리 단일 감지 트리거)**: Idle/Searching에서 Combat(스크림 인트로) 진입을 감지 반경(`IsTargetInDetectionRange`)만으로 판정하는 규칙. 장애물/시야선(LOS) 여부와 무관하게 거리 조건만 충족하면 전투 전환이 발생한다. current boss follow-up에서는 이 판정 전에 `Aggro Priority Retarget`이 먼저 current target을 최신 aggro rule로 맞춘다.
