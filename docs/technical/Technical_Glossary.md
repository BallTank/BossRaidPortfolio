...
* **MultiplayerPlayerPresentationDriver**: `Assets/Scripts/Multiplayer/Gameplay/MultiplayerPlayerPresentationDriver.cs`에 있는 multiplayer local presentation 전용 driver. current code에서는 `PredictedLocomotion` owner path의 local presentation, owner render proxy, predicted render trace를 소유한다. 2026-03-30 follow-up 기준 free move owner 화면은 raw root를 그대로 보지 않고, visual child와 camera가 같은 `render proxy` surface를 보도록 연결한다.
...
* **Boundary Sync (경계 동기화)**: 이전 tick에서는 free-move prediction이 가능했지만 이번 tick에서는 불가능해지는 전환을 경계로 보고, 다음 authoritative snapshot에서 즉시 동기화하는 규칙. current 1차 구현에서는 attack/dash 입력 시작이나 `MoveState` 이탈을 이 방식으로 처리한다.
...
* **Client Prediction Movement Trace**: real `PredictionReconciliation` owner path를 읽기 위한 runtime log. `[MultiplayerClientMoveTrace]` prefix로 `predict`, `fallback`, `boundarySync`, `hardFail`, `staleSync`, `idleSettle`, `shadow`, `defer` 등을 기록한다. `seq`, `root/auth/predicted position`, `posError`, `yawError`, `replayCount`를 읽어 owner correction이 free move 지연 상태인지, 즉시 동기화 상태인지 분리해서 본다. 다만 current `posError / yawError` 는 `same-sequence predicted state vs authoritative state` 비교값이므로, `on-screen root가 render frame에서 부드러운가` 를 직접 뜻하지는 않는다. old log에서 `hardFailShadow` 가 보이면 removed follow-up 이전 build인지 먼저 확인한다.
* **Predicted Render Trace Hook**: owner free move visual이 raw root target과 얼마나 벌어지는지 확인하기 위한 local diagnostic log. `[MultiplayerPredictedRenderTrace]` prefix로 `root / target / visual / visualTargetOffset / rootYaw / visualVelMag` 와 `tickStep / behindTicks / interpMode / smoothWindow / alphaFloor / linearAlpha / interpAlpha / supportSmooth` 를 기록한다. current shared render proxy path에서는 `visualTargetOffset` 이 `proxy가 raw root에서 얼마나 뒤에 있는가` 를 뜻할 수 있다. 이 로그는 `proxy lag` 와 `body follow` 확인에는 좋지만, correction correctness 자체는 `[MultiplayerClientMoveTrace]`, camera가 같은 proxy를 보는지는 `[MultiplayerCameraFollowTrace]` 와 같이 읽어야 한다.
* **Multiplayer Camera Follow Trace**: current owner 화면 jitter를 직접 읽기 위해 `ThirdPersonCameraController` 에 추가한 새 runtime log. `[MultiplayerCameraFollowTrace]` prefix로 `anchor / desired / camera` 좌표와 `anchorPlanarDelta / desiredPlanarDelta / cameraPlanarDelta / anchorStillFrames / cameraToAnchorPlanar / cameraToDesired` 등을 기록한다. 목적은 `camera가 따라가는 root anchor가 몇 frame 멈췄다가 한 tick에서 튀는가`, `camera smoothing이 그 jump를 얼마나 흡수하는가` 를 render frame 기준으로 보는 것이다.
* **Predicted Owner Tight Follow**: predicted owner camera path에서 render proxy orbit을 다시 부드럽게 늦게 쫓지 않도록, active position/rotation smoothing을 더 직접적으로 두는 follow-up 규칙. current 기본값은 `posSmooth=0`, `rotSmooth=0` 이고, `MultiplayerCameraFollowTrace` 에도 이 active 값이 그대로 기록된다.
