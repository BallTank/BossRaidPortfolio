...
* **Shared CharacterController Locomotion Core (`PlayerLocomotionCore`)**: Path B `Phase 2` foundation에서 시작한 shared locomotion capture / apply / simulate entry point. current refactor slice에서는 `Assets/Scripts/Player/PlayerLocomotionCore.cs`로 분리되어, Host sim과 owner prediction이 같은 `CharacterController`-based move / rotate / gravity / grounded rule을 계속 공유한다. `PlayerController`는 compatibility wrapper만 유지한다.
* **MultiplayerPlayerPresentationDriver**: `Assets/Scripts/Multiplayer/Gameplay/MultiplayerPlayerPresentationDriver.cs`에 있는 multiplayer local presentation 전용 driver. current code에서는 `PredictedLocomotion` owner path의 predicted render smoothing, predicted render trace, related runtime state만 소유한다. 2026-03-30 cleanup으로 old `LookOnly` masking / camera follow masking / presentation trace branch는 제거했다.
...
* **PredictionReconciliation**: current multiplayer movement runtime path. owner client는 locomotion input을 즉시 predict하고, Host authority replica는 shared `PlayerLocomotionCore`로 같은 입력을 authoritative하게 sim한 뒤 snapshot을 내려준다. owner는 baseline sync 뒤 deadzone check, reconcile, replay를 수행한다.
...
* **Predicted Render Smoothing**: `PredictedLocomotion` owner path에서 gameplay root나 reconciliation logic는 건드리지 않고, visible body child만 render-frame에서 부드럽게 따라오게 하는 좁은 보정층. pure `A / D` strafe처럼 `phase=predict`가 대부분인데도 남는 slight jitter를 `render-side tick-step visibility`로 보고 줄이기 위한 follow-up이다. current refactor slice에서는 이 로직이 `MultiplayerPlayerPresentationDriver`로 이동했다.
