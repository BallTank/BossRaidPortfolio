...
* **LookOnly Simulation Mode**: `PlayerController.RuntimeSimulationMode`의 local presentation 모드. local camera/look cache는 유지하지만 movement/attack simulation은 Host가 담당한다. 2026-03-26 current active path에서는 local owner visual child와 camera follow position에만 Boss Room style masking을 적용한다. body visual position은 `idle -> move start` frame snap을 유지하고, `big error = snap`, `moving medium-large error = faster temporary smooth catch-up`, 나머지 idle/catch-up 구간에서는 normal smooth catch-up을 사용한다. local owner locomotion `Speed` animator parameter는 즉시 갱신해 facing-to-run delay를 줄이도록 보강했다.
...
* **PredictedLocomotion Simulation Mode**: owner client 전용 locomotion prediction 모드. local move/rotate는 즉시 실행하지만 final truth는 Host snapshot이 가진다. 2026-03-26 `Path B Phase 3` 기준으로 `PredictionReconciliation` path에서 실제 사용된다.
* **AuthoritativeLocomotion Simulation Mode**: Host 전용 fixed-tick locomotion authority 모드. owner client와 같은 move/rotate/gravity helper를 사용하지만 final state를 결정하는 쪽은 Host다. 2026-03-26 `Path B Phase 4` 기준으로 `PredictionReconciliation` path에서 실제 사용된다.
* **Authoritative Locomotion Snapshot (`MultiplayerLocomotionState`)**: `inputSequence`, `serverTick`, `position`, `yaw`, `planarVelocity`, `verticalVelocity`, `jumpTimer`, grounded/prediction flag를 담는 직렬화 struct. current reset path에서는 owner replay 기준이라기보다, Host가 current authoritative transform/velocity 상태를 owner에게 내려주는 snapshot으로 쓴다.
...
* **Shared CharacterController Locomotion Core**: Path B `Phase 2`에서 `PlayerController` 안으로 추출한 shared locomotion capture / apply / simulate entry point. Host sim과 owner prediction이 같은 `CharacterController`-based move / rotate / gravity / grounded rule을 호출하도록 만들기 위한 foundation이며, `PredictionReconciliation` path의 locomotion-only scope가 이 core를 공유한다.
...
* **Correction Deadzone**: owner predicted state와 Host authoritative state 차이가 아주 작을 때는 rewind/replay correction을 건너뛰는 규칙. current slight jitter를 줄이기 위한 small safe fix다.
...
* **Host-Only CharacterController Path**: current multiplayer movement truth. client는 입력을 Host로 보내고, Host는 solo와 같은 `MoveState` / `CharacterController.Move()` 경로로 실제 움직임을 계산한 뒤 owner에게 authoritative snapshot만 내려준다. easy English flow는 `client sends input -> Host simulates -> client applies Host snapshot`이다.
* **LocomotionRuntimePath**: `MultiplayerPlayerAvatar`가 갖는 runtime path switch enum. `HostOnlyCharacterController`와 `PredictionReconciliation`을 구분한다. 2026-03-26 `Path B Phase 3~5` 기준으로 `PredictionReconciliation` path는 locomotion-only owner prediction + Host authority + reconciliation/replay를 실제로 실행할 수 있고, current default는 여전히 `HostOnlyCharacterController`다.
* **Verify Prefab Path Override**: `Assets/Resources/Multiplayer/MultiplayerPlayerAvatar.prefab`가 `_locomotionRuntimePath = PredictionReconciliation`를 serialize해, `Assets/Scenes/mutiplayer/GamePlayScene_Verify.unity` runtime spawn이 manual inspector switch 없이 바로 Path B를 타게 만든 override.
* **Runtime Role Configuration Log**: `MultiplayerPlayerAvatar` spawn 시 남기는 diagnostic log. `selectedPath / activePrediction / server / owner / mode`를 한 줄로 출력해, current test가 Host-only path인지 real `PredictionReconciliation` path인지 바로 구분하기 위한 용도다.
* **Client Prediction Movement Trace**: real `PredictionReconciliation` owner path를 읽기 위한 runtime log. `[MultiplayerClientMoveTrace]` prefix로 `predict`, `fallback`, `reconcile`를 기본 출력하고, 필요할 때만 `deadzone / duplicate` packet trace를 다시 켤 수 있게 정리했다. `seq`, `root/auth/predicted position`, `posError`, `yawError`, `replayCount`를 기록해 client-screen movement가 prediction 쪽인지 authoritative correction 쪽인지 분리해서 읽는다.
* **Predicted Render Trace Hook**: pure `A / D` strafe처럼 남은 slight jitter가 render-side인지 확인하기 위한 local owner diagnostic log. `[MultiplayerPredictedRenderTrace]` prefix로 `root / target / visual / visualTargetOffset / rootYaw / visualVelMag`를 기록하며, current follow-up에서는 `tickStep / behindTicks / interpMode / smoothWindow / alphaFloor / linearAlpha / interpAlpha / supportSmooth`도 함께 찍어 visible body가 현재 몇 tick 정도 뒤처졌는지와 cubic eased alpha가 얼마나 빨리 catch-up하는지 바로 읽게 했다. latest reading says `behindTicks` alone is not enough; `visualVelMag`도 함께 읽어야 actual visual calmness를 판단할 수 있다. current default는 lateral-dominant input에서만 읽도록 제한한다.
* **Predicted Render Tick Interpolation**: `PredictedLocomotion` local owner visual child가 raw predicted root를 `SmoothDamp`로 chase하는 대신, predicted tick state 사이를 render frame에서 보간하는 presentation rule. current active shape는 pure locomotion 전 방향에서 같은 `previous predicted tick -> current predicted tick` interpolation을 사용하고, interpolation alpha는 stronger faster-early cubic ease-out curve로 계산한다. latest narrow follow-up은 tick-boundary frame이 exact `alpha=0`에서 시작하지 않도록 small `alphaFloor = 0.05`를 먼저 적용한다. large gap는 snap으로 처리한다. latest grouped visual reading says low `alphaFloor` zone (`0.04 ~ 0.07`) is calmer on screen than bigger values.
...
* **Predicted Render Smoothing**: `PredictedLocomotion` owner path에서 gameplay root나 reconciliation logic는 건드리지 않고, visible body child만 render-frame에서 부드럽게 따라오게 하는 좁은 보정층. pure `A / D` strafe처럼 `phase=predict`가 대부분인데도 남는 slight jitter를 `render-side tick-step visibility`로 보고 줄이기 위한 follow-up이다.
* **Boss Room-Style Latency Masking**: gameplay root/collider truth는 Host authoritative snapshot에 그대로 맡기고, local owner가 보는 visual child와 camera follow anchor에만 보정을 적용하는 층. current branch에서는 gameplay movement truth를 갈라지게 하지 않고 visible jitter를 줄이기 위한 안전한 follow-up으로 쓴다. current active path에서는 body position은 `idle -> move start` 순간의 snap을 유지하고, `big error = snap`, `moving medium-large error = faster temporary smooth catch-up`, idle/catch-up small error에서는 normal smooth catch-up 규칙을 사용한다. `active move = direct follow` 실험과 broader `strong move-angle change` response window 실험은 local client jitter를 키워 제거했다. latest narrow follow-up은 local owner body facing/animator response만 더 빠르게 만드는 쪽으로 범위를 줄인 상태다.
* **Multiplayer Presentation Trace Hook**: local owner `LookOnly` presentation layer를 읽기 위한 임시 진단 로그. current branch에서는 old Host-only presentation path를 다시 볼 때만 쓰는 fallback diagnostic으로 남겨두고, default는 off다.
* **Medium Moving Catch-Up (중간 이동 보정)**: trace log에서 `move-start snap` 이후 moving 초반 `visualTargetOffset`이 약 `0.22 ~ 0.46`까지 다시 커지는 패턴을 바탕으로 넣은 local owner body-position rule. `direct follow` 대신, moving 중 visual child가 medium-large distance만큼 뒤처졌을 때만 더 짧은 smooth time으로 따라붙게 한다.
...
* **Legacy Scene Player Handoff**: multiplayer gameplay scene이 시작될 때 scene에 남아 있는 old `Player`를 그대로 쓰지 않고, main camera를 먼저 분리한 뒤 spawn snapshot을 캐시하고 runtime hierarchy에서 legacy player를 제거한 다음 network player avatar로 넘기는 handoff 절차. cleanup entry는 `SceneManager.sceneLoaded`와 avatar spawn fallback 두 경로로 보강돼 있다.
