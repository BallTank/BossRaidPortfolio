...
public class PlayerController : MonoBehaviour, IDashContext, IAttackable, IBossAttackHitReceiver
{
    ...
    private MultiplayerPlayerPresentationDriver EnsureMultiplayerPresentationDriver()
    {
        if (_multiplayerPresentationDriver == null)
        {
            _multiplayerPresentationDriver = new MultiplayerPlayerPresentationDriver(this);
        }

        return _multiplayerPresentationDriver;
    }
    ...
    {
        return PlayerLocomotionCore.CaptureCurrentState(this, inputSequence, serverTick, allowsPrediction);
    }
    ...
    {
        return EnsureMultiplayerPresentationDriver().GetPreferredCameraFollowPosition();
    }
